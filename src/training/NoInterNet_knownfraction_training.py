...
def main(ns):
    ...
    Pks, labels, lenk = NINf.load_Pkf_feat(ns.input, ns.fraction_path, ns.k_max, ns.k_min, ns.log, ns.multipole, ns.norm_fs, ns.norm_c)

    #maximin normalization
    max_Pks = Pks[:,:-1].max(axis=0) #f (Pks[:,-1]) is already normalized
    min_Pks = Pks[:,:-1].min(axis=0)
    Pks[:,:-1] = NINf.maxmin_corr(Pks[:,:-1], max_Pks, min_Pks)
    norm = np.ones(len(Pks))

    n_train = int(len(Pks)*ns.train_fraction)
    n_val = int(len(Pks)*ns.val_fraction)
    ...
    #preparing model
    model = NINf.NoInterNet_fraction_compress_knowf_inference(input_size, ns.neurons, output_size, n_out, ns.n_min).to(device)
        
    optimizer = torch.optim.Adam(model.parameters(), lr=ns.learning_rate)
    
    #training
    train_history, val_history = NINf.training_inference_knowf(train_dataloader, val_dataloader, model, optimizer, ns.epochs, ns.patience, ns.output, lenk, ns.neurons)
