...
def main_real(ns):
    ...
    for i in range(first, last):
        ...
        if ns.inbox:
            output_complete = os.path.join(ns.output, f'{ns.dz:.2f}', f_name, f'{use_i:d}')
        else:
            output_complete = os.path.join(ns.output, f'{ns.dz:.2f}-iz{ns.snapnum_dz}', f_name, f'{use_i:d}')
        ...
        if ns.step == 3:
            Bk_name = f'Bk_pyspectrum_f_{f:.2f}.dat' if all_f else 'Bk_pyspectrum.dat'
        else:
            Bk_name = f'Bk_{ns.step:d}k_pyspectrum_f_{f:.2f}.dat' if all_f else f'Bk_{ns.step:d}k_pyspectrum.dat'

        cBk.analyse_FoF(pos_d, output_complete, snapdir, ns.snapnum, grid, ns.boxsize, dz=ns.dz, name=Bk_name)

    message = "Varying fraction " if all_f else f"Fraction = {fractions:1.3f} "
...
def main_RSD(ns):
    ...
    for i in range(first, last):
        ...
        if ns.inbox:
            output_complete = os.path.join(ns.output, f'{ns.dz:.2f}', f_name, f'{use_i:d}')
            name_t = f'Bk_{ns.step:d}k_pyspectrum_-no-dz.dat'
        else:
            output_complete = os.path.join(ns.output, f'{ns.dz:.2f}-iz{ns.snapnum_dz}', f_name, f'{use_i:d}')
        if not os.path.exists(output_complete):
            os.makedirs(output_complete)
...
if __name__ == "__main__":
    desc = "Compute and save the Bk of the box in fixed cosmology with displacemente with or without RSD and either varying or fixed fraction"
