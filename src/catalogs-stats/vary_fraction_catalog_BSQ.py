...
def main(ns):
    ...
    for i in range(first, last):
        ...
        f = fractions[i]
        if ns.inbox:
            output_complete = os.path.join(ns.output, f'inbox', 'BSQ', f'{i:d}') 
        else:
            output_complete = os.path.join(ns.output, f'outbox', 'BSQ', f'{i:d}')
        ...
        output_complete = os.path.join(output_complete, f'{ns.frealization:d}', f'groups_0{ns.snapnum:02d}')
        if not os.path.exists(output_complete):
            os.makedirs(output_complete)
        ...
        FoF_name = f'group_tab_0{ns.snapnum:02d}.0'
        
        readfof.writeFoFCatalog(FoF_c, os.path.join(output_complete, FoF_name))
        
if __name__ == "__main__":
    desc = "Produce and save the contaminated catalogues in cosmology big Sobol sequence case"
    ...
    parser.add_argument('--output', default='/mustfs/LAPP-DATA/theorie/cagliari/Quijote_BSQ/FoF_contaminated/', help=h)
